...
function AppContent() {
  ...
  const handleDecision = async (action) => {
    ...
    try {
      ...
      const advisoryResponse = await fetch('http://localhost:5000/api/ask-twin', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleAskTwin = async (doubt, voiceFeatures = null) => {
    ...
    try {
      ...
      if (voiceFeatures) {
        const voiceEmotionResponse = await fetch('http://localhost:5000/api/analyze-voice-emotion', {
          method: 'POST',
